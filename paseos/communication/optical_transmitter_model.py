...
class OpticalTransmitterModel(TransmitterModel):
    ...
    def __init__(
        ...
        assert antenna_gain > 0 or antenna_diameter > 0 or fwhm > 0, (
            "Antenna gain or antenna diameter or " "FWHM needs to be higher than 0."
        )
        assert (
                sum(param != 0 for param in (antenna_diameter, antenna_gain, fwhm)) <= 1
        ), ("Only set one of antenna gain, " "antenna diameter, and FWHM not multiple.")

        self.input_power = input_power
        self.power_efficiency = power_efficiency
        self.output_power = 10 * math.log10(
            input_power * power_efficiency * 1000
        )  # dBm
        self.FWHM = fwhm

    def set_gain(self) -> None:
        """Sets gain for a transmitter, based on the given gain, or antenna diameter and
        wavelength."""
