...
def test_link_creation():
    ...
    with pytest.raises(AssertionError, match="A radio receiver is required for this radio link."):
        link = RadioLinkModel(sat_actor, radio_name, comms_sat, optical_receiver_name)
...
def test_transmitter_creation():
    ...
    with pytest.raises(AssertionError, match="Antenna efficiency should be between 0 and 1."):
        ActorBuilder.add_comm_device(actor=sat_actor,device_name=radio_name, input_power=2, power_efficiency=0.5,
                                antenna_efficiency=1.5, line_losses=1, point_losses=5, antenna_diameter=0.3, device_type=DeviceType.RADIO_TRANSMITTER)
...
def test_radio_transmitter_creation():
    ...
    with pytest.raises(AssertionError, match="Antenna gain or antenna diameter needs to be higher than 0."):
        ActorBuilder.add_comm_device(actor=sat_actor,device_name=radio_name, input_power=2, power_efficiency=0.5,
                                antenna_efficiency=0.5, line_losses=1, point_losses=5, device_type=DeviceType.RADIO_TRANSMITTER)

    with pytest.raises(AssertionError, match="Only set one of antenna gain and antenna diameter, not both."):
        ActorBuilder.add_comm_device(actor=sat_actor,device_name=radio_name, input_power=2, power_efficiency=0.5,
                                antenna_efficiency=0.5, antenna_diameter=1, antenna_gain=100, line_losses=1, point_losses=5, 
...
def test_optical_transmitter_creation():
    ...
    with pytest.raises(AssertionError, match="Antenna gain or antenna diameter or FWHM needs to be higher than 0."):
        ActorBuilder.add_comm_device(actor=sat_actor,device_name="optical_transmitter_name", input_power=1, power_efficiency=1, 
                                 antenna_efficiency=1, line_losses=1, point_losses=3, device_type=DeviceType.OPTICAL_TRANSMITTER)
    ...
    with pytest.raises(AssertionError, match="Only set one of antenna gain, antenna diameter, and FWHM not multiple."):
        ActorBuilder.add_comm_device(actor=sat_actor,device_name="optical_transmitter_name", input_power=1, power_efficiency=1, 
                                 antenna_efficiency=1, line_losses=1, point_losses=3, antenna_gain=100, antenna_diameter=1,
                                 device_type=DeviceType.OPTICAL_TRANSMITTER)
    with pytest.raises(AssertionError, match="Only set one of antenna gain, antenna diameter, and FWHM not multiple."):
        ActorBuilder.add_comm_device(actor=sat_actor,device_name="optical_transmitter_name", input_power=1, power_efficiency=1, 
    ...
    with pytest.raises(AssertionError, match="Only set one of antenna gain, antenna diameter, and FWHM not multiple."):
        ActorBuilder.add_comm_device(actor=sat_actor,device_name="optical_transmitter_name", input_power=1, power_efficiency=1, 
                                 antenna_efficiency=1, line_losses=1, point_losses=3, FWHM=1E-3, antenna_diameter=1, antenna_gain=100,
                                 device_type=DeviceType.OPTICAL_TRANSMITTER)

    ActorBuilder.add_comm_device(actor=sat_actor,device_name="optical_transmitter_name", input_power=1, power_efficiency=1, 
                                 antenna_efficiency=1, line_losses=1, point_losses=3, FWHM=1E-3, device_type=DeviceType.OPTICAL_TRANSMITTER)
