...
from paseos.utils.reference_frame_transfer import compute_transformation_matrix_rpy_body
import trimesh
import numpy as np
...
def test_1():
    ...
    T = calculate_aero_torque(r, v, mesh, np.array([np.pi/4, np.pi/6, 0]), spacecraft_temperature_K)
...
def test_2a():
    ...
    T = calculate_aero_torque(r, v, mesh, np.array([0, 0, 0]), spacecraft_temperature_K)
...
def test_2b():
    ...
    r = np.array([6626, 0, 0])  # [km]
    v = np.array([0, 7756, 0])  # [km]
    T = calculate_aero_torque(r, v, mesh, np.array([0, np.pi/6, 0]), spacecraft_temperature_K)
...
def test_2c():
    ...
    T = calculate_aero_torque(r, v, mesh, np.array([np.pi/6, 0, 0]), spacecraft_temperature_K)
...
def test_2d():
    ...
    T = calculate_aero_torque(r, v, mesh, np.array([0, 0, np.pi/6]), spacecraft_temperature_K)
