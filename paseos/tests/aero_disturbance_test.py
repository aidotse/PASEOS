...
from paseos.attitude.disturbance_torques_utils import compute_aerodynamic_torque
...
def test_1():
    ...
    T = compute_aerodynamic_torque(
        r, v, mesh, np.array([np.pi / 4, np.pi / 6, 0]), spacecraft_temperature_K
    )
...
def test_2b():
    ...
    v = np.array([0, 7756, 0])  # [km]
    T = compute_aerodynamic_torque(
        r, v, mesh, np.array([0, np.pi / 6, 0]), spacecraft_temperature_K
    )
...
def test_2c():
    ...
    T = compute_aerodynamic_torque(
        r, v, mesh, np.array([np.pi / 6, 0, 0]), spacecraft_temperature_K
    )
...
def test_2d():
    ...
    T = compute_aerodynamic_torque(
        r, v, mesh, np.array([0, 0, np.pi / 6]), spacecraft_temperature_K
    )
